...
throws-like
  ...
  'fails with wrong argument @kind';
throws-like
  { Math::FFT::Libfftw3::R2R.new: data => 1..6, kind => (FFTW_R2HC,FFTW_R2HC,FFTW_R2HC)},
  X::Libfftw3,
  message => /Argument ' ' \@kind ' ' must ' ' have/,
  'fails when @kind has wrong dimensions';
subtest {
...
subtest {
  ...
  throws-like
    { Math::FFT::Libfftw3::R2R.new: data => @array, kind => (FFTW_HC2R, FFTW_HC2R) },
    X::Libfftw3, message => /Array ' ' of ' ' arrays/,
    'fails if no dims present';
  my Math::FFT::Libfftw3::R2R $fft .= new: data => @array, dims => (6,3), kind => (FFTW_R2HC, FFTW_R2HC);
  cmp-ok $fft.rank,    '==', 2, 'rank of data vector/matrix';
  cmp-ok $fft.dims[0], '==', 6, 'first dimension of vector/matrix';
  cmp-ok $fft.dims[1], '==', 3, 'second dimension of vector/matrix';
  is-deeply $fft.in,
...
subtest {
  ...
  my Math::FFT::Libfftw3::R2R $fft .= new: data => @array, kind => (FFTW_R2HC, FFTW_R2HC);
  cmp-ok $fft.rank,    '==', 2, 'rank of data vector/matrix';
  cmp-ok $fft.dims[0], '==', 6, 'first dimension of vector/matrix';
  cmp-ok $fft.dims[1], '==', 3, 'second dimension of vector/matrix';
...
subtest {
  if (try require Math::Matrix) !=== Nil {
    ...
    my Math::FFT::Libfftw3::R2R $fft .= new: data => $matrix, kind => (FFTW_R2HC, FFTW_R2HC);
    cmp-ok $fft.rank,    '==', 2, 'rank of data vector/matrix';
    cmp-ok $fft.dims[0], '==', 6, 'first dimension of vector/matrix';
    cmp-ok $fft.dims[1], '==', 3, 'second dimension of vector/matrix';
    my @out;
